...
package org.toradocu.doclet.internal.toolkit.taglets;

import com.sun.javadoc.*;
import org.toradocu.doclet.internal.toolkit.*;
import org.toradocu.doclet.internal.toolkit.util.*;
...
public abstract class TagletWriter {
   ...
   */
  protected abstract Content paramTagOutput(ParamTag paramTag, String paramName);
   ...
   */
  protected abstract Content simpleTagOutput(Tag[] simpleTags, String header);
   ...
   */
  protected abstract Content valueTagOutput(
      FieldDoc field, String constantVal, boolean includeLink);
   ...
   */
  public static void genTagOuput(
      TagletManager tagletManager, Doc doc, Taglet[] taglets, TagletWriter writer, Content output) {
   ...
   */
  public static Content getInlineTagOuput(
      TagletManager tagletManager, Tag holderTag, Tag inlineTag, TagletWriter tagletWriter) {
    ...
    for (int j = 0; j < definedTags.length; j++) {
      if (("@" + definedTags[j].getName()).equals(inlineTag.name())) {
        ...
        Content output =
            definedTags[j]
                .getTagletOutput(
                    holderTag != null && definedTags[j].getName().equals("inheritDoc")
                        ? holderTag
                        : inlineTag,
                    tagletWriter);
        return output;
   ...
   * @return the {@link Content} representing the comments.
   */
  public abstract Content commentTagsToOutput(
