 ...
 * questions.
 */
package org.toradocu.doclet.formats.html;
...
import org.toradocu.doclet.internal.toolkit.AbstractDoclet;
import org.toradocu.doclet.internal.toolkit.Configuration;
import org.toradocu.doclet.internal.toolkit.builders.AbstractBuilder;
import org.toradocu.doclet.internal.toolkit.util.ClassTree;
import org.toradocu.doclet.internal.toolkit.util.DocFile;
import org.toradocu.doclet.internal.toolkit.util.DocPath;
import org.toradocu.doclet.internal.toolkit.util.DocPaths;
import org.toradocu.doclet.internal.toolkit.util.DocletAbortException;
import org.toradocu.doclet.internal.toolkit.util.IndexBuilder;
import org.toradocu.doclet.internal.toolkit.util.Util;
...
public class HtmlDoclet extends AbstractDoclet {
  ...
  protected void generateClassFiles(ClassDoc[] arr, ClassTree classtree) {
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      try {
        ...
        Toradocu.process(
            curr, configuration); // This is the only change made to the standard doclet.
      } catch (IOException e) {
  ...
  protected void generateProfileFiles() throws Exception {
    if (configuration.showProfiles && configuration.profilePackages.size() > 0) {
      ...
      for (int i = 1; i < configuration.profiles.getProfileCount(); i++) {
        ...
        if (!configuration.shouldDocumentProfile(profileName)) continue;
        ...
        for (int j = 0; j < packages.length; j++) {
          ...
          if (!(configuration.nodeprecated && Util.isDeprecated(packages[j]))) {
            ...
            next =
                (j + 1 < packages.length && packages[j + 1].name().length() > 0)
                    ? packages[j + 1]
                    : null;
            AbstractBuilder profilePackageSummaryBuilder =
                configuration
                    .getBuilderFactory()
                    .getProfilePackageSummaryBuilder(packages[j], prev, next, Profile.lookup(i));
            profilePackageSummaryBuilder.build();
            prev = packages[j];
          }
        }
        nextProfile =
            (i + 1 < configuration.profiles.getProfileCount()) ? Profile.lookup(i + 1) : null;
        AbstractBuilder profileSummaryBuilder =
            configuration
                .getBuilderFactory()
                .getProfileSummaryBuilder(Profile.lookup(i), prevProfile, nextProfile);
      ...
      if (!(configuration.nodeprecated && Util.isDeprecated(packages[i]))) {
        ...
        if (configuration.createtree) {
          PackageTreeWriter.generate(
              configuration, packages[i], prev, next, configuration.nodeprecated);
        }
        prev = packages[i];
      }
    }
   ...
   *
   * <p>This method is not required and will default gracefully (to true) if absent.
   *
  ...
  private void performCopy(String filename) {
    ...
    try {
      ...
      if (toFile.isSameFile(fromfile)) return;
