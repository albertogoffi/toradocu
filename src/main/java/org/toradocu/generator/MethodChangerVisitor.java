...
public class MethodChangerVisitor
  ...
  private void getExpectedExceptionChanger(
    ...
    for (ThrowsSpecification throwsSpecification : operationSpec.getThrowsSpecifications()) {
      ...
      conditionExpression = StaticJavaParser.parseExpression(condition);
      ...
      ifStmt.setThenStmt(StaticJavaParser.parseBlock(addExpectedException));
      ...
      ClassOrInterfaceType nullPointerException =
    		  StaticJavaParser.parseClassOrInterfaceType("java.lang.NullPointerException");
      CatchClause catchClause =
          new CatchClause(new Parameter(nullPointerException, "e"), StaticJavaParser.parseBlock("{}"));
      NodeList<CatchClause> catchClauses = new NodeList<>();
      catchClauses.add(catchClause);

      TryStmt nullCheckTryCatch = new TryStmt();
      nullCheckTryCatch.setTryBlock(StaticJavaParser.parseBlock("{" + ifStmt.toString() + "}"));
  ...
  private static IfStmt createIfStmt(
    ...
    conditionExpression = StaticJavaParser.parseExpression(condition);
    ifStmt.setCondition(conditionExpression);
    ifStmt.setThenStmt(StaticJavaParser.parseBlock(thenBlock));
    if (!elseBlock.isEmpty()) {
      ifStmt.setElseStmt(StaticJavaParser.parseBlock(elseBlock));
    }
