...
echo "RETURN PRECISION,=INDIRECT(\"L\" & ROW()-9)/(INDIRECT(\"L\" & ROW()-9)+INDIRECT(\"M\" & ROW()-9)+INDIRECT(\"M\" & ROW()-9))" >> $STATS_FILE
echo "RETURN RECALL,=INDIRECT(\"N\" & ROW()-10)/(INDIRECT(\"L\" & ROW()-10)+INDIRECT(\"M\" & ROW()-10)+INDIRECT(\"O\" & ROW()-10))" >> $STATS_FILE
