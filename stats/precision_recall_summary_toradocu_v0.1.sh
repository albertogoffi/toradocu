...
=SUM(N1:INDIRECT(\"N\" & ROW()-1)),=SUM(O1:INDIRECT(\"O\" & ROW()-1)),=SUM(P1:INDIRECT(\"P\" & ROW()-1)),," >> $STATS_FILE
...
echo "NUMBER OF CONDITIONS,=INDIRECT(\"N\" & ROW()-8)+INDIRECT(\"O\" & ROW()-8)+INDIRECT(\"P\" & ROW()-8)" >> $STATS_FILE
echo "OVERALL PRECISION,=INDIRECT(\"N\" & ROW()-9)/(INDIRECT(\"N\" & ROW()-9) + INDIRECT(\"O\" & ROW()-9))" >> $STATS_FILE
echo "OVERALL RECALL,=INDIRECT(\"N\" & ROW()-10)/INDIRECT(\"B\" & ROW()-2)" >> $STATS_FILE
